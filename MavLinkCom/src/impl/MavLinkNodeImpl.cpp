		...
		if (message.msgid == MavLinkCommandAck::kMessageId)
		{
			...
			if (ack.command == cmd)
			{
				...
				if (ackResult == MAV_RESULT::MAV_RESULT_TEMPORARILY_REJECTED) {
					Utils::logMessage("### command %d result: MAV_RESULT_TEMPORARILY_REJECTED", cmd);
				}
				else if (ackResult == MAV_RESULT::MAV_RESULT_UNSUPPORTED) {
					Utils::logMessage("### command %d result: MAV_RESULT_UNSUPPORTED", cmd);
				}
				else if (ackResult == MAV_RESULT::MAV_RESULT_FAILED) {
					Utils::logMessage("### command %d result: MAV_RESULT_FAILED", cmd);
				}
				else if (ackResult == MAV_RESULT::MAV_RESULT_ACCEPTED) {
					Utils::logMessage("### command %d result: MAV_RESULT_ACCEPTED", cmd);
				}
				else {
					Utils::logMessage("### command %d unexpected result: %d", cmd, ackResult);
				}
