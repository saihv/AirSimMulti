...
class VehicleCameraConnector : public msr::airlib::VehicleCamera
{
	APIPCamera* camera;
	int id;
public:
	...
	bool getScreenShot(msr::airlib::DroneControllerBase::ImageType imageType, std::vector<uint8_t>& compressedPng) override
	{

		if (this->camera == nullptr) {
			return false;
		}
		if (imageType == msr::airlib::DroneControllerBase::ImageType::None) {
			return false;
		}
		...
		compressedPng = std::vector<msr::airlib::uint8_t>(image.GetData(), image.GetData() + image.Num());
		return true;
	}
};